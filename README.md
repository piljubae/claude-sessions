...
Claude Code는 세션 단위로 작동하기 때문에, 새 세션을 시작하면 이전 작업 맥락이 초기화된다. Jira 티켓 하나를 여러 세션에 걸쳐 진행할 때마다 "지난번에 어디까지 했더라"를 다시 설명하는 비용이 크다.

이 도구는 세션을 자동으로 분류·요약해 두고, 새 세션 시작 시 관련 이전 세션을 컨텍스트로 제공함으로써 그 비용을 줄인다.
